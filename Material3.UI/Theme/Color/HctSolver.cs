...
/// <summary>
/// Solves the unique ARGB whose CAM16-rendered hue and chroma match a target HCT and whose
/// L* equals the given tone. Implements the Newton-Raphson + bisection method from the Material
/// Color Utilities Java reference.
/// </summary>
internal static class HctSolver
{
    ...
    /// <summary>Sanitizes a small enough angle in radians to be coterminal in [0, 2pi).</summary>
    private static double SanitizeRadians(double angle) => (angle + Math.PI * 8) % (Math.PI * 2);

    /// <summary>Delinearizes a 0..100 linear-RGB component to a 0..255 floating-point value.</summary>
    ...
    private static double InverseChromaticAdaptation(double adapted)
    {
        ...
        double bas = Math.Max(0, 27.13 * adaptedAbs / (400.0 - adaptedAbs));
        return MathUtils.Signum(adapted) * Math.Pow(bas, 1.0 / 0.42);
    }
    ...
    private static double[] LerpPoint(double[] source, double t, double[] target) => new[]
    {
    ...
    private static double[] NthVertex(double y, int n)
    {
        ...
        else
        {
            ...
            return IsBounded(b) ? new[] { r, g, b } : new[] { -1.0, -1.0, -1.0 };
        }
    }
    ...
    private static double[] Midpoint(double[] a, double[] b) => new[]
    {
        (a[0] + b[0]) / 2.0,
        (a[1] + b[1]) / 2.0,
        (a[2] + b[2]) / 2.0,
    ...
    private static int FindResultByJ(double hueRadians, double chroma, double y)
    {
        ...
        for (int iterationRound = 0; iterationRound < 5; iterationRound++)
        {
            ...
            double[] linrgb = MathUtils.MatrixMultiply(
                new[] { rCScaled, gCScaled, bCScaled }, LINRGB_FROM_SCALED_DISCOUNT);
    ...
    /// <summary>Solves for an ARGB whose CAM16 hue/chroma match the target and whose L* equals tone.</summary>
    ...
    /// <summary>Solves for a CAM16 whose hue/chroma match the target and whose L* equals tone.</summary>
